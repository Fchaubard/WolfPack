...
<div data-role="content">
		...
		if($task=="join"){ //Joining an existing open group:
			...
			try{
			$queryString = "UPDATE users SET groupNumber=".$_SESSION['groupId'].", status=1 WHERE userName=\"".$_SESSION['userName']."\"";
		...
		if($task=="create"){ //Creating a new group:
			...
			//Add group number and status of group owner to current session user:
				$query3 = "UPDATE users SET groupNumber=".$_SESSION['groupId'].", status=3 WHERE userName=\"".$un."\"";		$result = mysql_query($query3)or die(mysql_error());
			...
			for($i=1;$i<=$buttonCount;$i++){
				...
				if($value=="on"){
					$friendGroupQuery = "UPDATE users SET groupNumber=".$_SESSION['groupId'].", status=2 WHERE userName=\"".$name."\"";
		...
		if($task=="banana"){ //Add more to group:
			$queryForAdd = "SELECT * FROM users WHERE userName=\"".$_SESSION['userName']."\"";
		 	...
		 	while($row = mysql_fetch_array($result)){
		 		$_SESSION['groupId'] = $row['groupNumber'];
    		}
...
<?php
    ...
    $query = "SELECT * FROM groups WHERE id=\"".$_SESSION['groupId']."\"";
    $result = mysql_query($query)or die(mysql_error());
    while($row = mysql_fetch_array($result)){
        $groupName = $row['name'];
    }
